...
static void beep(uint16_t freq, uint16_t dur) {
  (void)freq; (void)dur;  // M5StickC has no built-in buzzer
}
...
static void drawSettings() {
  ...
  int mw = 72, mh = 16 + SETTINGS_N * 10 + MENU_HINT_H;
  ...
  for (int i = 0; i < SETTINGS_N; i++) {
    ...
    spr.setCursor(mx + 6, my + 6 + i * 10);
    spr.print(sel ? "> " : "  ");
    spr.print(settingsItems[i]);
    spr.setCursor(mx + mw - 24, my + 6 + i * 10);
    spr.setTextColor(p.textDim, PANEL);
    if (i == 0) {
...
static void drawReset() {
  const Palette& p = characterPalette();
  int mw = 72, mh = 16 + RESET_N * 10 + MENU_HINT_H;
  int mx = (W - mw) / 2, my = (H - mh) / 2;
  spr.fillRoundRect(mx, my, mw, mh, 4, PANEL);
...
void drawMenu() {
  const Palette& p = characterPalette();
  int mw = 72, mh = 16 + MENU_N * 10 + MENU_HINT_H;
  int mx = (W - mw) / 2, my = (H - mh) / 2;
  spr.fillRoundRect(mx, my, mw, mh, 4, PANEL);
  spr.drawRoundRect(mx, my, mw, mh, 4, p.textDim);
  spr.setTextSize(1);
...
static void drawClock() {
  ...
  if (clockOrient == 0) {
    paintedOrient = 0;
    // Bottom half — buddy naturally lives at y=0..55, GIF peeks at top
    // via peek mode. Clearing from 60 leaves both untouched.
    spr.fillRect(0, 60, W, H - 60, p.bg);
    spr.setTextDatum(MC_DATUM);
    spr.setTextSize(2); spr.setTextColor(p.text, p.bg);    spr.drawString(hm, CX, 93);
    spr.setTextSize(1); spr.setTextColor(p.textDim, p.bg); spr.drawString(ss, CX, 115);
                                                            spr.drawString(dl, CX, 132);
  ...
  if (repaint || _clkTm.Seconds != lastSec) {
    ...
    M5.Lcd.setTextSize(2); M5.Lcd.setTextColor(p.text, p.bg);    M5.Lcd.drawString(hm, 113, 20);
    M5.Lcd.setTextSize(1); M5.Lcd.setTextColor(p.textDim, p.bg); M5.Lcd.drawString(ssl, 113, 42);
                                                                  M5.Lcd.drawString(wdl, 113, 58);
  ...
  if (millis() - lastPetTick >= 200) {
    ...
    if (buddyMode) {
      ...
      characterRenderTo(&M5.Lcd, 34, 27);
    }
  }
...
void drawPasskey() {
  ...
  spr.setTextSize(1);
  spr.setTextColor(p.textDim, p.bg);
  spr.setCursor(8, 37);  spr.print("BT PAIRING");
  spr.setCursor(8, 122); spr.print("enter code:");
  spr.setTextSize(2);
  spr.setTextColor(p.text, p.bg);
  char b[8]; snprintf(b, sizeof(b), "%06lu", (unsigned long)blePasskey());
  spr.setCursor((W - 6 * 12) / 2, 73);
  spr.print(b);
}
...
void drawInfo() {
  ...
  auto ln = [&](const char* fmt, ...) {
    ...
    spr.setCursor(4, y); spr.print(b); y += 7;
  };
  ...
  if (infoPage == 0) {
    ...
    spr.setTextSize(2);
    spr.setCursor(4, y);
    spr.print(linked ? "linked" : (settings().bt ? "on" : "off"));
    spr.setTextSize(1);
    y += 20;
...
static void drawApproval() {
  ...
  // Size 2 only if it fits one line (~5 chars at 12px on 80px screen)
  int toolLen = strlen(tama.promptTool);
  spr.setTextColor(p.text, p.bg);
  spr.setTextSize(toolLen <= 5 ? 2 : 1);
  spr.setCursor(4, H - AREA + (toolLen <= 5 ? 12 : 14));
  spr.print(tama.promptTool);
  spr.setTextSize(1);

  // Hint wraps at ~11 chars to two lines under the tool name
  spr.setTextColor(p.textDim, p.bg);
  int hlen = strlen(tama.promptHint);
  spr.setCursor(4, H - AREA + 28);
  spr.printf("%.11s", tama.promptHint);
  if (hlen > 11) {
    spr.setCursor(4, H - AREA + 35);
    spr.printf("%.11s", tama.promptHint + 11);
  }
...
static void drawPetStats(const Palette& p) {
  ...
  for (int i = 0; i < 4; i++) tinyHeart(38 + i * 10, y + 2, i < mood, moodCol);
  ...
  for (int i = 0; i < 5; i++) {
    int px = 38 + i * 8;
    if (i < en) spr.fillRect(px, y - 2, 6, 6, enCol);
    else spr.drawRect(px, y - 2, 6, 6, p.textDim);
  }

  y += 16;
  spr.fillRoundRect(6, y - 2, 36, 12, 3, p.body);
  spr.setTextColor(p.bg, p.body);
  spr.setCursor(10, y + 1); spr.printf("Lv %u", stats().level);
  ...
  tokFmt("tokens   ", stats().tokens, y + 24);
  tokFmt("today    ", tama.tokensToday, y + 32);
}
...
static void drawPetHowTo(const Palette& p) {
  ...
  auto ln = [&](uint16_t c, const char* s) {
    spr.setTextColor(c, p.bg); spr.setCursor(6, y); spr.print(s); y += 7;
  };
  auto gap = [&]() { y += 2; };

  y += 9;  // room for the PET header drawn by drawPet()
...
void drawHUD() {
  ...
  const Palette& p = characterPalette();
  const int SHOW = 3, LH = 8, WIDTH = 11;
...
void setup() {
  ...
  M5.Lcd.setRotation(0);
  M5.Imu.Init();
  // M5StickC has no built-in buzzer — no Beep.begin() needed
  startBt();
