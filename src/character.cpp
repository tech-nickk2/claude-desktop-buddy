...
// Peek mode pins the GIF bottom to the info-panel top (y=46) so the pet
// sits on the panel edge regardless of canvas height. Home mode centers
// in the upper 90px. No padding assumed in the source art.
static const int   PEEK_TOP = 46;
...
// the whole pet fits the 46px window instead of cropping the top.
static void gifPlace() {
  int outW = peekMode ? gifW / 2 : gifW;
  int outH = peekMode ? gifH / 2 : gifH;
  gifX = (spr.width() - outW) / 2;
  gifY = peekMode ? (PEEK_TOP - outH) / 2 : (90 - outH) / 2;
}
